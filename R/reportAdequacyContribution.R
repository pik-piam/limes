...
reportAdequacyContribution <- function(gdx) {
  ...
  for (te2 in te) {
    capadeq[,,te2] <- v_cap[,,te2]*p_tedata_nu[,,te2]*p_adeq_te[,,te2]
  }
  ...
  tmp1 <- mbind(tmp1,setNames(dimSums(capadeq[,,c("hydro","ror","hs")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Hydro (GW)"))
  tmp1 <- mbind(tmp1,setNames(dimSums(capadeq[,,c("pc","pc80","pc95","pc10","pcc","lpc","lpc80","lpc95","lpc10","lpcc")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Coal (GW)"))
  tmp1 <- mbind(tmp1,setNames(dimSums(capadeq[,,c("pc","pc80","pc95","pc10","pcc")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Hard Coal (GW)"))
  tmp1 <- mbind(tmp1,setNames(dimSums(capadeq[,,c("lpc","lpc80","lpc95","lpc10","lpcc")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Lignite (GW)"))
  tmp1 <- mbind(tmp1,setNames(dimSums(capadeq[,,c("ngcc","ngcc80","ngcc95","ngcc10","ngt","ngccc")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Gas (GW)"))
  tmp1 <- mbind(tmp1,setNames(dimSums(capadeq[,,c("biolcigcc")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Biomass (GW)"))
  #tmp1 <- mbind(tmp1,setNames(dimSums(0.8*capadeq[,,c("psp","batteries","helec")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Storage (GW)")) 
  tmp1 <- mbind(tmp1,setNames(dimSums(capadeq[,,c("oil")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Oil (GW)"))
  tmp1 <- mbind(tmp1,setNames(dimSums(capadeq[,,c("waste")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Waste (GW)"))
  tmp1 <- mbind(tmp1,setNames(dimSums(capadeq[,,c("hcc","hct","hfc")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Hydrogen (GW)"))
  tmp1 <- mbind(tmp1,setNames(dimSums(capadeq[,,c("others")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Other (GW)"))
  ...
  capadeq_vres_marg <- v_cap[,,ter]*0
  capadeq_vres_peak <- capadeq_vres_marg
  
  capadeq_stor_marg <- v_cap[,,testore]*0
  #capadeq_stor_marg <- setNames(capadeq_stor_marg,NULL)
  capadeq_stor_peak <- capadeq_stor_marg
  
  demand_marg <- p_eldemand[,,1]*0
  demand_marg <- setNames(demand_marg,NULL)
  demand_peak <- demand_marg
  
  netimports_marg <- p_eldemand[,,1]*0
  netimports_marg <- setNames(netimports_marg,NULL)
  ...
  for (regi2 in getRegions(p_eldemand)) {
    for (year2 in getYears(p_eldemand)) { 
      if (taupeak[regi2,year2,] == 0) {
        ...
        capadeq_vres_peak[regi2,year2,] <- p_adeq_te[,,ter]*v_seprodmax[regi2,year2,as.character(taupeak[regi2,year2,])]
        demand_peak[regi2,year2,] <- p_eldemand[regi2,year2,as.character(taupeak[regi2,year2,])]
        netimports_peak[regi2,year2,] <- max(0, p_eldemand[regi2,year2,as.character(taupeak[regi2,year2,])] + dimSums(v_storein[regi2,year2,as.character(taupeak[regi2,year2,])], dim=3) - dimSums(v_storeout[regi2,year2,as.character(taupeak[regi2,year2,])], dim=3) - dimSums(v_seprod[regi2,year2,as.character(taupeak[regi2,year2,])], dim=3))
        capadeq_stor_peak[regi2,year2,] <- p_adeq_te[,,testore]*v_storeout[regi2,year2,as.character(taupeak[regi2,year2,])]
      }
      ...
      if (taumax[regi2,year2,] == 0) {
        ...
        capadeq_vres_marg[regi2,year2,] <- p_adeq_te[,,ter]*v_seprodmax[regi2,year2,as.character(taumax[regi2,year2,])]
        demand_marg[regi2,year2,] <- p_eldemand[regi2,year2,as.character(taumax[regi2,year2,])]
        netimports_marg[regi2,year2,] <- max(0, p_eldemand[regi2,year2,as.character(taumax[regi2,year2,])] + dimSums(v_storein[regi2,year2,as.character(taumax[regi2,year2,])], dim=3) - dimSums(v_storeout[regi2,year2,as.character(taumax[regi2,year2,])], dim=3) - dimSums(v_seprod[regi2,year2,as.character(taumax[regi2,year2,])], dim=3))
        capadeq_stor_marg[regi2,year2,] <- p_adeq_te[,,testore]*v_storeout[regi2,year2,as.character(taumax[regi2,year2,])]
      }
  ...
  tmp3 <- mbind(tmp3,setNames(dimSums(capadeq_stor_marg[,,c("psp","batteries","helec")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Storage (GW)"))
  
  #Contribution of storage (peak demand)
  tmp3 <- mbind(tmp3,setNames(dimSums(capadeq_stor_peak[,,c("psp","batteries","helec")],dim=3),"Capacity Adequacy|Peak Demand|Contribution|Storage (GW)"))
  ...
  tmp5 <- mbind(tmp5,setNames(dimSums(v_capreserve[,,c(tegas)],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Reserve Plants|Gas (GW)"))
  tmp5 <- mbind(tmp5,setNames(dimSums(v_capreserve[,,c(tegas)],dim=3),"Capacity Adequacy|Peak Demand|Contribution|Reserve Plants|Gas (GW)"))
  tmp5 <- mbind(tmp5,setNames(dimSums(v_capreserve[,,c(tengcc)],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Reserve Plants|Gas CC (GW)"))
  tmp5 <- mbind(tmp5,setNames(dimSums(v_capreserve[,,c(tengcc)],dim=3),"Capacity Adequacy|Peak Demand|Contribution|Reserve Plants|Gas CC (GW)"))
  tmp5 <- mbind(tmp5,setNames(dimSums(v_capreserve[,,c("ngt")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Reserve Plants|Gas OC (GW)"))
  tmp5 <- mbind(tmp5,setNames(dimSums(v_capreserve[,,c("ngt")],dim=3),"Capacity Adequacy|Peak Demand|Contribution|Reserve Plants|Gas OC (GW)"))
  tmp5 <- mbind(tmp5,setNames(dimSums(v_capreserve[,,c("biolcigcc")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Reserve Plants|Biomass (GW)"))
  tmp5 <- mbind(tmp5,setNames(dimSums(v_capreserve[,,c("biolcigcc")],dim=3),"Capacity Adequacy|Peak Demand|Contribution|Reserve Plants|Biomass (GW)"))
  tmp5 <- mbind(tmp5,setNames(dimSums(v_capreserve[,,c("oil")],dim=3),"Capacity Adequacy|Most Challenging|Contribution|Reserve Plants|Oil (GW)"))
  tmp5 <- mbind(tmp5,setNames(dimSums(v_capreserve[,,c("oil")],dim=3),"Capacity Adequacy|Peak Demand|Contribution|Reserve Plants|Oil (GW)"))
